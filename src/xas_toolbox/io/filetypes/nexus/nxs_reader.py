...
class NxsReader:
    ...
    Attrs:
        ...
        elementMeta (dict): Empty dictionary to be given paths to <i>symbol, edge, reference</i>
                            and <i>ref_edge</i>.
        transParams (dict): Empty dictionary to be given paths to <i>mutrans, energy, i0</i> and
                            <i>itrans</i>.
        fluorParams (dict): Empty dictionary to be given paths to <i>mufluor, energy</i> and 
                            <i>ifluor</i>.
    ...
    def _get_ScanData(self, val:ScanData)->Union[np.ndarray, None]:
        """
        ...
        tosearch = _val.mode
        if tosearch is None: tosearch = "monoParams"
        if tosearch:
    ...
    def _get_ScanMeta(self, val:ScanMeta)->Union[str, list]:
        """
        ...
        if val == "path": return self.scanMeta["path"]
        ...
        if hasattr(out, "ndim"):
            if out.ndim == 0:
                ...
                if out == [""]: out = None #repetition files need to be split by \n
        return out
    
    def _get_ElementMeta(self, val:ElementMeta)->Union[str, None]:
        """
        ...
        _path = self.elementMeta[val][0]; xml_name = self.elementMeta[val][-1]
    ...
    def get_value(self, val:Union[ScanData, ElementMeta, ScanMeta])\
                        -> Union[np.ndarray, str, list, None]:
