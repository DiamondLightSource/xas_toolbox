...
class NxsReader:
    ...
    Attrs:
        ...
        elementMeta (dict): Empty dictionary to be given paths to <i>symbol, edge,
          reference</i>
                            and <i>ref_edge</i>.
        transParams (dict): Empty dictionary to be given paths to <i>mutrans, energy,
          i0</i> and
                            <i>itrans</i>.
        fluorParams (dict): Empty dictionary to be given paths to <i>mufluor, e
        nergy</i> and
                            <i>ifluor</i>.
    ...
    def __init__(self, path: Path):
        ...
        self.monoParams = {"i0": None, "energy": None, "time": None}
    ...
    def get_value(
        ...
        if val in ScanData.__members__.keys():
            ...
            return self._get_element_meta(val)

        elif val in ScanMeta.__members__.keys():
            return self._get_scan_meta(val)
