...
class XdiReader:
    ...
    def _get_colmap(self, col_labels: list) -> dict:
        ...
        for k in ScanData:
            vname = k.valname
            colval = list(filter(lambda col: vname in col.lower(), col_labels))
            if len(colval) < 1:
                if vname in alt_names.keys():
                    colval = list(
                        filter(lambda col: alt_names[vname] in col.lower(), col_labels)
                    )
                else:
    ...
    def _populate_colmap(self) -> None:
        ...
        i0 = self._colmap["i0"]
        if i0:
            i0 = self._scandata[i0]
        it = self._colmap["itrans"]
        if it:
            it = self._scandata[it]
        iff = self._colmap["ifluor"]
        if iff:
            iff = self._scandata[iff]
        ir = self._colmap["irefer"]
        if ir:
            ir = self._scandata[ir]

        for k, v in self._colmap.items():
            if (k in ScanMeta.__members__.keys()) or (
                k in ElementMeta.__members__.keys()
            ):
                continue
            if v is not None:
    ...
    def _read_from_columns(self) -> None:
        ...
        header = header.split("\n")
        cols = cols.split("\n")
        col_labels = list(filter(lambda v: "column" in v.lower(), header))
        ...
        for k, v in {
            ...
            "edge": "element.edge",
            "ref_symbol": "element.reference",
            "ref_edge": "element.ref_edge",
        }.items():
            tmp = [e for e in elm_labels if v in e.lower()]
            if len(tmp) >= 1:
                self._colmap[f"{k}"] = tmp[0].split(": ")[-1]
            else:
                self._colmap[f"{k}"] = None

        dpattern = r"\d+.\d+"
        scandata = [
            np.array(re.findall(dpattern, f), dtype=np.float64) for f in cols[:-1]
        ]
    ...
    def get_value(self, val: ScanData | ScanMeta | ElementMeta) -> np.ndarray | None:
        ...
        if val not in self._colmap.keys():
            logger.info(f"{val} not found in scan.")
            return None
        if self._colmap[val] is not None:
            out = self._colmap[val]
        else:
            logger.info(f"{val} not present in scan.")
