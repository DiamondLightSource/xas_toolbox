...
def interpolate_data(
    x: np.ndarray, y: np.ndarray, xtarget: np.ndarray, method: str = "Pchip"
) -> np.ndarray:
    """
    ...
    Returns:
        fx (np.ndarray): Interpolated `y` data onto `xtarget`.
    """
    ...
    if "pchip" in method.lower():
        if len(y.shape) == 1:
            ...
            fx = PchipInterpolator(x, y, axis=1)(xtarget)  # noqa: E701
    elif method.lower() == "linear":
        if len(y.shape) == 1:
            fx = interp1d(x, y, kind="linear", fill_value="extrapolate")(xtarget)
        else:
            fx = interp1d(x, y, kind="linear", axis=1, fill_value="extrapolate")(
                xtarget
            )
    else:
        raise NotImplementedError(
            f"{method} not available (current options: `Pchip`, \
                                  `linear`)"
        )
    return fx
