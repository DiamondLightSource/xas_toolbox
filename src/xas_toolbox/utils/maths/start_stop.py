...
def _get_start_stop(coords:np.ndarray|list, diff:np.ndarray, min_spacing:float)\
                                                            ->list[tuple]:
    """
    ...
    Parameters:
        ...
        diff (np.ndarray): list of differences between some array to filter the coords based-off.
    ...
    while start < size:
        slice = lookup[start:]
        if (np.all(slice)): ss.append((coords[start], coords[-1]+1)); break
        stop = np.argmin(slice)+start
        ss.append((coords[start], coords[stop]+1)); start=stop+1
        if start == size: ss.append((coords[start],coords[start]+1))
    return ss

def _mean_start_stop(coords:np.ndarray|list, diff:np.ndarray, min_spacing:float=50)->np.ndarray:
    """
    ...
    for i in range(len(ss_full)): averaged[i] = int(np.mean(ss_full[i]))
