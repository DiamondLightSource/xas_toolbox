...
def _index_iupac_lines(edge: str) -> list:
    edge_len = len(edge)
    return [l for l in _iupac_lines if l[:edge_len]]  # noqa: E741


def _get_transition_dict(edge: str) -> tuple[list[int], list[str]]:
    rel_lines = _index_iupac_lines(edge)
    out = ([getattr(xraylib, l) for l in rel_lines], rel_lines)  # noqa: E741
    return out


_transitions = {
    edge: _get_transition_dict(edge) for edge in ["K", "L1", "L2", "L3", "M"]
}  # noqa: E501
...
def to_density(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        if "rho" in b_args.arguments.keys() and b_args.arguments["rho"] is not None:
            b_args.arguments["surf_dens"] = (
                b_args.arguments["rho"] * b_args.arguments["t"]
            )  # noqa: E501
