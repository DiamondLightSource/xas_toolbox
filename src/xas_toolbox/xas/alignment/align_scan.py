...
from xas_toolbox.xas.edges import calc_e0, compare_e0s

logger = logging.getLogger(__name__)


def calc_energy_shift(
    ...
    if x.ndim == 1:
        if y.ndim == 1:
            ...
            if delta_e == 0:
                logger.info("No shift from predicted e0 value.")
            x_out = x + delta_e
        else:
            raise NotImplementedError(
                "Unable to align stacks of reference absorption data."
            )
    else:
        x_out = x.copy()
...
def align_scan(scan: XasMeasurement) -> XasMeasurement:
    ...
    if hasattr(scan, "meta"):
        ...
        raise AttributeError("Scan has no meta-data.")

    if symbol is None:
        logger.info("No absorbing atom in metadata.")
    if edge is None:
        logger.info("No absorbing edge in meta-data.")
