...
def get_target_energy(x: np.ndarray) -> np.ndarray:
    ...
    """  # noqa: E501
    startvals = np.nanmin(x, axis=1)
    endvals = np.nanmax(x, axis=1)
    e_start = np.max(startvals)
    e_stop = np.min(endvals)

    if e_stop <= e_start:
        raise ValueError("Scans too dissimilar to correct energy axis.")

    Edensity = 0  # noqa: N806
    for i in range(x.shape[0]):
        masked = np.ma.masked_invalid(x[i, :]).compressed()

        density = (masked[-1] - masked[0]) / len(masked)
        if density >= Edensity:
            Edensity = density  # noqa: N806
