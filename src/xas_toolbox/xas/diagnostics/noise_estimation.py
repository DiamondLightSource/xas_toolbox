...
from larch.xafs import autobk, estimate_noise, set_xafsGroup
from scipy.signal import savgol_filter

from xas_toolbox.io import XasMeasurement


def noise_estimate_poly(
    y: np.ndarray, winlength: int = 7, polyorder: int = 3
) -> np.ndarray | float:
    """
    ...
    if y.ndim > 1:
        n_scans = y.shape[0]
        noise = np.zeros(n_scans)  # noqa: E702
...
def noise_estimate_fft(
    ...
    if y.ndim > 1:
        ...
        for i in range(y.shape[0]):
            ...
            estimate_noise(group_tmp.k, group_tmp.chi, group=noise_group_tmp, kweight=2)
...
def snr_metrics(
    data: Group | XasMeasurement,
) -> tuple[np.ndarray | float, np.ndarray | float, np.ndarray | float, Group, Group]:
    """
    Get estimates of noise and kmax for a series of spectra using fitting with a
    polynomial
    or larch's `estimate_noise` function.

    Arguments:
        data (Group|XasMeasurement): Stack of spectra to analyze, they must share a
          common energy axis.

    Returns:
        tuple (tuple): tuple containing:
            noise_poly (np.ndarray|float): Estimated noise from subtracting a polynomial
              from scans.
    ...
    if data.energy.ndim > 1:
        raise NotImplementedError("Cannot produce snr metrics for un-aligned scans.")
