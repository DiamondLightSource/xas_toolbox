...
def plot_repetition_metrics(data:Group):
    
    flat_mean = data.flat_avr; flat_std = data.flat.std(axis=0)
    ...
    ax[0,0].plot(x, y); ax[0,0].plot(x, pre, label="pre-edge")
    ...
    ax[0,1].set_title("Flattened average"); ax[0,1].plot(x, flat_mean)
    ax[1,0].plot(k, chi*k**2, label=r"$\chi$"); ax[1,0].legend()
    ax[1,0].set_xlabel(r"k ($\AA^{-1}$)"); ax[1,0].set_ylabel(r"$k^2\cdot\chi$ ($\AA^{-2}$)")
    ax[1,1].plot(x, y, label="mu"); ax[1,1].plot(x, bkg, label="autobk"); ax[1,1].legend(fontsize="x-small")
    [a.set_xlabel("Energy (eV)") for a in [ax[0,0], ax[0,1], ax[1,1]]]

    e0s = data.e0
    fig, ax = plt.subplots(2, 1, figsize=(10, 8), layout="tight")
    ax[0].set_title("Edge Position"); ax[0].set_xlabel("Repetition Number")
    ax[0].set_ylabel("Edge Position (eV)")
    ax[0].plot(np.arange(1,len(e0s)+1, 1) ,e0s, "o")

    ax[1].set_title("Flattened Variation"); ax[1].set_xlabel("Energy (eV)")
    ax[1].plot(x, flat_mean, label="average")
    ax[1].fill_between(x, flat_mean+flat_std, flat_mean-flat_std, color="r", label=r"$\pm\sigma^2$")
    ax[1].legend()

    ncomp = 2
    model = PCA(n_components=ncomp)
    W = model.fit_transform(data.flat); H = model.components_
    fig, ax = plt.subplots(ncomp,2,figsize=(10, 8), layout="tight")
    fig.suptitle("PCA")
    for i in range(ncomp):
        ax[i,0].set_title(f"PC{i + 1} Scores"); ax[i,0].set_xlabel('Repetition Number')
        ax[i,0].scatter(np.arange(1,W.shape[0]+1), W[:,i])
        
        ax[i,1].set_title(f"PC{i + 1} Loadings"); ax[i,1].set_xlabel("Energy (eV)")
        ax[i,1].plot(x,H[i,:])
