...
def calc_e0(
    ...
    Returns:
        tuple (tuple): tuple containing:
            ...
            edge_coords (int | np.ndarray[int]): List of indices in x where edges occur.
    ...
    if y.ndim == 1:
        yf = np.gradient(gaussian_filter(y, sigma))
        yf /= np.max(yf)
    else:
        yf = np.gradient(gaussian_filter(y, sigma), axis=1)
        yf /= np.max(yf, axis=1)[:, None]

    yf = np.round(yf, 0)
    yf = np.diff(yf, prepend=0)
    if y.ndim == 1:
        edge_coords = np.where(yf == 1)[0]
        ...
        edge_coords = [np.where(yf[i, :] == 1)[0] for i in range(y.shape[0])]
        edge_coords = np.apply_along_axis(
            partial(_filter_edge_coords, energy=x, min_spacing=post), 1, edge_coords
        )
        edge_energy = [x[e] for i in range(y.shape[0]) for e in edge_coords[i]]
        edge_number = [len(e) for e in edge_coords]

    return edge_energy, np.array(edge_coords, dtype=int), edge_number
...
def edge_from_symbol(x: np.ndarray, y: np.ndarray, symbol: str) -> str:
    ...
    """
    Z = SymbolToAtomicNumber(symbol)  # noqa: N806
    e0, ec, eno = calc_e0(x, y)
    e0s = np.array([EdgeEnergy(Z, e.upper()) for e in _edges])
    diffs = np.abs(e0s - e0)
...
def compare_e0s(
    ...
    if edge is None:
        if symbol is None:
            symbol, edge = guess_edge(e0_calc)
        else:
            edge = edge_from_symbol(x, y, symbol)
...
def _filter_edge_coords(
    ...
    return np.array(averages, dtype=int)
