...
_round_to = lambda a, n: np.array([round(x, n) for x in a])
...
def find_edges(y:np.ndarray)->tuple[np.ndarray, np.ndarray]:
    ...
    Returns:
        tuple (tuple): tuple containing:
            where_edge (np.ndarray): Indices of y at the maximum derivative of a detected edge.
    ...
    dy = np.gradient(y)
    yf = _round_to(dy, 1)
    filt = lambda y: np.array([0 if i <= 0 else 1 for i in y])
    ny = filt(yf); dny = np.concat(([0], np.diff(ny)))
...
def get_edge_bounds(x:np.ndarray, y:np.ndarray, edge_energy:float)\
...
def fit_quad_bkg(x:np.ndarray, y:np.ndarray, bounds:tuple[int, int],
                 pre_idx:int, edge_idx:int, include:bool=True)->tuple[float, float, float]:
    """
